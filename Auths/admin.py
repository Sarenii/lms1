...
class CustomUserAdmin(UserAdmin):
    ...
    list_display = (
        "username", 
        "email", 
        "role",  # Include role in the list display
        "is_verified", 
        "is_staff", 
        "registration_date"
    )  # Columns to display in the user list
    list_filter = ("role", "is_verified", "is_staff", "is_superuser", "registration_date")  # Filters in the admin sidebar
    ...
    fieldsets = (
        ...
        ("Personal Info", {"fields": ("first_name", "last_name", "email", "profile_picture", "role")}),  # Add role field
    ...
    add_fieldsets = (
        (None, {
            "classes": ("wide",),
            "fields": ("username", "email", "password1", "password2", "first_name", "last_name", "role", "is_verified"),  # Add role field
        }),
    )
